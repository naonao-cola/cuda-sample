...
extern "C" __global__ void flash_attention_spilling_from_registers_k(float* out, float* out_l, float* Q, float* K, float* V, float scaling, int n, int T_r, int T_c);

extern "C" __global__ void flash_attention_k(float* out, float* out_l, float* Q, float* K, float* V, float scaling, int n, int T_r, int T_c);
...
int main()
{
    ...
    for (int i = 0; i < n * d; i++)
        h_Q[i] = static_cast<float>(i % 13) * 0.1f;
    ...
    // Kernel launch
    flash_attention_spilling_from_registers_k<<<blocks, threads>>>(d_out, d_out_l, d_Q, d_K, d_V, scaling, n, T_r, T_c);
    ...
    // Kernel launch
    flash_attention_k<<<blocks, threads>>>(d_out, d_out_l, d_Q, d_K, d_V, scaling, n, T_r, T_c);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
