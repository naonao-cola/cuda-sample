...
extern void initialData(float* in, const int size);
...
extern void checkResult(float* hostRef, float* gpuRef, const int size);
...
void transposeRectangle()
{
    ...
    dim3 grid2((nx + block.x * 2 - 1) / (block.x * 2), (ny + block.y - 1) / block.y);
    ...
    float ibnd = 2 * nx * ny * sizeof(float) / (1024.0 * 1024.0 * 1024.0) / iElaps;
    ...
    iStart = seconds();
    transposeSmemPadDyn<<<grid, block, (BDIMX + IPAD) * BDIMY * sizeof(float)>>>(d_C, d_A, nx, ny);
    CHECK(cudaDeviceSynchronize());
    iElaps = seconds() - iStart;
    ...
    iStart = seconds();
    transposeSmemUnrollPadDyn<<<grid2, block, (BDIMX * 2 + IPAD) * BDIMY * sizeof(float)>>>(d_C, d_A, nx, ny);
    CHECK(cudaDeviceSynchronize());
    iElaps = seconds() - iStart;
